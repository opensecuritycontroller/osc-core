...
public class PortGroupExtendedHookCheckTask extends TransactionalMetaTask {

    private final Logger log = Logger.getLogger(PortGroupExtendedHookCheckTask.class);
    ...
    public PortGroupExtendedHookCheckTask(SecurityGroupMember sgm, SecurityGroupInterface bindedSGI, VMPort vmPort,
            VmDiscoveryCache vdc) {
        this.log.info(String.format("PortGroupExtendedHookCheckTask sgm: %s sgi: %s vmPort: %s ", sgm, bindedSGI, vmPort));
        this.sgm = sgm;
    ...
    public void executeTransaction(EntityManager em) throws Exception {
        ...
        this.log.info("AssignedRedirectedDai " + assignedRedirectedDai);
        ...
        if (!this.securityGroupInterface.getMarkedForDeletion()) {
            if (assignedRedirectedDai == null) {
                ...
                if (this.sgm.getType().equals(SecurityGroupMemberType.SUBNET) && this.sgm.getSubnet().isProtectExternal()
                    ...
                    if (SdnControllerApiFactory.supportsOffboxRedirection(this.sgm.getSecurityGroup())) {
                        assignedRedirectedDai = OpenstackUtil.findDeployedDAI(em, this.vs, this.sgm.getSecurityGroup(),
                                tenantId, getMemberRegion(this.sgm), sgmDomainId, null);
                    } else {
                    ...
                    assignedRedirectedDai = OpenstackUtil.findDeployedDAI(em, this.vs, this.sgm.getSecurityGroup(),
                            tenantId, getMemberRegion(this.sgm), sgmDomainId, this.vmPort.getVm().getHost());
                }
            ...
            InspectionHookElement hook = null;
            try (SdnControllerApi controller = SdnControllerApiFactory.createNetworkControllerApi(assignedRedirectedDai)) {
                ...
                String fwdPolicyId = this.securityGroupInterface.getNetworkElementId();
                if (fwdPolicyId != null) {
                    DeploymentSpec ds = assignedRedirectedDai.getDeploymentSpec();
                    String osDomainId = OpenstackUtil.extractDomainId(ds.getTenantId(), ds.getTenantName(),
                            ds.getVirtualSystem().getVirtualizationConnector(), new ArrayList<NetworkElement>(
                            Arrays.asList(ingressPort)));
                    if (osDomainId != null){
                        ingressPort.setParentId(osDomainId);
                        egressPort.setParentId(osDomainId);
                    }
                    DefaultNetworkPort fwdPolicy = new DefaultNetworkPort();
                    fwdPolicy.setElementId(fwdPolicyId);
                    //can also pass inspectedPort from which PortGroup can be fetched in the plugin
                    hook = controller.getInspectionHook(fwdPolicy, new DefaultInspectionPort(ingressPort, egressPort));
                }
            }

            // Missing tag indicates missing hook
            if (hook == null || (hook.getTag() == null && this.securityGroupInterface.getTag() != null)) {
                this.tg.addTask(new PortGroupHookCreateTask(this.sgm.getSecurityGroup(), this.securityGroupInterface,
                        this.vs, assignedRedirectedDai));
            } else {
