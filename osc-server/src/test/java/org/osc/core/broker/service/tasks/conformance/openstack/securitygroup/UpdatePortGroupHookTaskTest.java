...
public class UpdatePortGroupHookTaskTest extends BasePortGroupHookTaskTest {
    ...
    public void testExecute_WhenSGHasNoNetworkElementId_ThrowsValidationException() throws Exception {
        super.testExecute_WhenSGHasNoNetworkElementId_ThrowsValidationException(false);
    }

    @Test
    public void testExecute_WhenCreateNetworkRedirectionApiFails_ThrowsTheUnhandledException() throws Exception {
        super.testExecute_WhenCreateNetworkRedirectionApiFails_ThrowsTheUnhandledException(false);
    }
    ...
    public void testExecute_WhenUpdateInspectionHookSucceeds_ExecutionFinishes() throws Exception {
        ...
        // Assert.
        Mockito.verify(redirectionApi, Mockito.times(1)).updateInspectionHook(argThat(new InspectionHookMatcher(dai, sgi)));
    }

    private SdnRedirectionApi mockUpdateInspectionHook(SecurityGroupInterface sgi, DistributedApplianceInstance dai, Exception e) throws Exception {
    ...
    private class InspectionHookMatcher extends ArgumentMatcher<InspectionHookElement> {
        ...
        public boolean matches(Object object) {
            ...
            return inspectionPortMatchesDAI(inspectionPort, this.dai) &&
                    inspectionHook.getInspectedPort().getElementId().equals(this.sgi.getSecurityGroup().getNetworkElementId()) &&
                    inspectionHook.getTag().equals(this.sgi.getTagValue()) &&
                    inspectionHook.getEncType().toString().equals(this.sgi.getVirtualSystem().getEncapsulationType().name()) &&
