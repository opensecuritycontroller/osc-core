...
public class CreatePortGroupHookTaskTest extends BasePortGroupHookTaskTest {
    ...
    public void testExecute_WhenSGHasNoNetworkElementId_ThrowsValidationException() throws Exception {
        super.testExecute_WhenSGHasNoNetworkElementId_ThrowsValidationException(true);
    }

    @Test
    public void testExecute_WhenCreateNetworkRedirectionApiFails_ThrowsTheUnhandledException() throws Exception {
        super.testExecute_WhenCreateNetworkRedirectionApiFails_ThrowsTheUnhandledException(true);
    }
    ...
    public void testExecute_WhenInstallInspectionHookReturnsNullId_ThrowsVmidcException() throws Exception {
        ...
        this.exception.expectMessage(String.format("The creation of the inspection hook for the security group interface %s."
    ...
    private SdnRedirectionApi mockInstallInspectionHook(SecurityGroupInterface sgi, DistributedApplianceInstance dai, Exception e) throws Exception {
        SdnRedirectionApi redirectionApi = mock(SdnRedirectionApi.class);
        when(redirectionApi.installInspectionHook(
                argThat(new NetworkElementMatcher(sgi.getSecurityGroup())),
                argThat(new InspectionPortElementMatcher(dai)),
                eq(Long.parseLong(sgi.getTag())),
                (TagEncapsulationType)isNull(),
                eq(sgi.getOrder()),
                (FailurePolicyType)isNull())).thenThrow(e);

        return redirectionApi;
    }

    private SdnRedirectionApi mockInstallInspectionHook(SecurityGroupInterface sgi, DistributedApplianceInstance dai, String inspectionHookId) throws Exception {
        SdnRedirectionApi redirectionApi = mock(SdnRedirectionApi.class);
        when(redirectionApi.installInspectionHook(
                argThat(new NetworkElementMatcher(sgi.getSecurityGroup())),
                argThat(new InspectionPortElementMatcher(dai)),
                ...
                (TagEncapsulationType)isNull(),
                eq(sgi.getOrder()),
                (FailurePolicyType)isNull())).thenReturn(inspectionHookId);
