...
public class BasePortGroupHookTaskTest {
    ...
    protected void testExecute_WhenSGHasNoNetworkElementId_ThrowsValidationException(boolean create) throws Exception {
        ...
        this.exception.expectMessage(String.format("The security group %s does not have a network element set.", sg.getName()));

        BasePortGroupHookTask task = create ? new CreatePortGroupHookTask(sgi, dai) : new UpdatePortGroupHookTask(sgi, dai);

        // Act.
        task.execute();
    }

    protected void testExecute_WhenCreateNetworkRedirectionApiFails_ThrowsTheUnhandledException(boolean create) throws Exception {
        // Arrange.
    ...
    protected SecurityGroupInterface registerNewSGI(SecurityGroup sg, Long entityId) {
        ...
        SecurityGroupInterface sgi = new SecurityGroupInterface(vs, null, entityId.toString(), FailurePolicyType.FAIL_CLOSE, 1L);
        sgi.setId(entityId);
