...
class PluginResolveContext extends ResolveContext {
	...
	// A cache of resource->location (URL), generated during resolve and queried after resolve in order to fetch the resource.
	private final Map<Resource, String> resourceLocationMap = new IdentityHashMap<>();
	// A cache of resources to the repositories which own them; used from insertHostedCapability method.
	...
	public Collection<Resource> getMandatoryResources() {
		return Collections.singleton(this.initialResource);
	}
	...
	public int insertHostedCapability(List<Capability> capabilities, HostedCapability hc) {
		...
		if (hostIndex == -1) {
            throw new IllegalArgumentException("Hosted capability has host resource not found in any known repository.");
        }
		...
		// The list passed by (some versions of) Felix does not support the single-arg add() method... this throws UnsupportedOperationException.
	...
	private static CapabilityImpl createInitialMarkerCapability(Resource resource) {
		return new CapabilityImpl(INITIAL_RESOURCE_CAPABILITY_NAMESPACE, Collections.emptyMap(), Collections.emptyMap(), resource);
	}
	...
	private static CapabilityImpl createIdentityCap(Resource resource, String identity) {
		...
		CapabilityImpl idCap = new CapabilityImpl(IdentityNamespace.IDENTITY_NAMESPACE, Collections.emptyMap(), idCapAttrs, resource);
		return idCap;
	}
