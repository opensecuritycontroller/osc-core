...
public class PluginResolveContextTest {
	...
	public void testMatchRequirementsEffectiveResolve() throws Exception {
		...
		assertTrue("effective should match: req=default, cap=resolve", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", null),
			createCapability("foo", "resolve", new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
		));
		assertTrue("effective should match: req=resolve, cap=default", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", "resolve"),
			createCapability("foo", null, new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
		));
		assertTrue("effective should match: req=resolve, cap=resolve", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", "resolve"),
			createCapability("foo", "resolve", new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
	...
	public void testEffectiveResolveCapsMatchAnyRequirement() throws Exception {
		...
		assertTrue("effective should match: req=blah, cap=default", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", "blah"),
			createCapability("foo", null, new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
		));
		assertTrue("effective should match: req=blah, cap=resolve", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", "blah"),
			createCapability("foo", "resolve", new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
	...
	public void testEffectiveNonResolveCapsMatchRequirementWithSame() throws Exception {
		...
		assertFalse("effective should NOT match: req=default, cap=blah", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", null),
			createCapability("foo", "blah", new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
		...
		assertFalse("effective should NOT match: req=wibble, cap=blah", PluginResolveContext.match(
			createRequirement("foo", "(foo=bar)", "wibble"),
			createCapability("foo", "blah", new HashMap<String,Object>() {{
				put("foo", "bar");
			}})
		));
